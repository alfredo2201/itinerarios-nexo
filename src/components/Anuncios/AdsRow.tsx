...
function AdsRow({ id, fileName,URL, rep, state, onUrlChange, deleteVideo}: Props) {
    ...
    return (
        <tr className="bg-white h-12 border-x-1 border-b-1 border-[#D9D9D9]">
            <td className="text-[10px] sm:text-[16px] font-normal px-3">{fileName}</td>
            <td className="text-[10px] sm:text-[16px] font-normal px-2 text-center"> {rep}</td>
            <td className="text-[10px] sm:text-[16px] font-normal px-2 text-center"> {state}</td>
            <td className="px-2 text-center ">
                <button 
                onClick={handleOnClickVideo} 
                className="bg-[#023672] w-25 rounded-full text-white text-[10px] sm:text-[14px] p-2 hover:bg-[#4185D4] cursor-pointer transition duration-150 ease-in-out">Ver Video</button>
            </td>
