...
export default function UserEditForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        await updateProfileAPI(formData, userToEdit?._id).then((res) => {
            if (res?.status == 200) {
                toast.success('Datos actualizados correctamente');
            }
        }).catch((e) => {
            toast.error('Error al actualizar este usuario: ', e);
        })
    };
    ...
    return (<>
        <form onSubmit={handleSubmit} className="p-8 dark:bg-gray-700">
            ...
            <div className="flex gap-4 pt-6 border-t border-gray-200 dark:border-gray-600">
                <button
                    type="submit"
                    className="flex-1 bg-[#023672] text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition shadow-lg hover:shadow-xl cursor-pointer"
                >
                    Guardar Usuario
                </button>
                <button
                    ...
                    className="flex-1 bg-gray-200 text-gray-700 dark:text-white dark:bg-gray-900 dark:hover:bg-gray-800 py-3 rounded-lg font-semibold hover:bg-gray-300 transition cursor-pointer"
                >
                    Cancelar
                </button>
            </div>
